...
pd = ParamMec(shear=shear, rho_rock=rho_roc, lenght_fault=lenght_fault, depth_fault=depth_fault, a_fric=a_fric,
              b_fric=b_fric, dc=dc, V_p=V_p, r_real=r_real, c_real=c_real, Pinf=Pinf)

# -------------------------------------#
# ND Mechanical parameter definition
# -------------------------------------#

pnd = NdParamMec(a=pd.a_fric / pd.b_fric, k=pd.k_rigidity * pd.dc / (pd.sigma_n0 * pd.b_fric),
                 eta=pd.eta_visc * pd.V_p / (pd.b_fric * pd.sigma_n0), psi=psi, f0=f0, b=pd.b_fric,
                 r=(pd.r_real * pd.b_fric * pd.sigma_n0) / (pd.shear * pd.dc),
                 c=pd.c_real * (pd.b_fric ** 2 * pd.sigma_n0 ** 2) / (pd.V_p * pd.shear ** 2 * pd.dc))
...
def phi_rns(t, phi, nu, sigma_n, pnd, delay):
    F = pnd.k * (1 - spsi * np.exp(phi)) * spsi - (np.exp(-nu) - np.exp(phi)) * (sigma_n - P(t, pd, pnd, delay)) + f_rns(phi,nu,pnd) * (pnd.k * (1 - spsi * np.exp(phi)) * cpsi - dP(t, pd, pnd,delay))
    F = F / (pnd.a * (sigma_n - P(t, pd, pnd,delay)) + pnd.eta * np.exp(phi))
...
def rkf(t, phi, nu, sigma_n, f, h, pnd, pc, delay):
    c21 = 1 / 4
    c31 = 3 / 32
    c32 = 9 / 32
    c41 = 1932 / 2197
    ...
    while passtep == 0 and iterrk <= pc.nitrkmax:
        ...
        k1 = h * phi_rns(t, phi, nu, sigma_n, pnd, delay)
        l1 = h * th_rns(phi, nu)
        ...
        k2 = h * phi_rns(t, phi + dphi, nu + dnu, sigma_n + dsigma, pnd, delay)
        ...
        k3 = h * phi_rns(t, phi + dphi, nu + dnu, sigma_n + dsigma, pnd, delay)
        ...
        k4 = h * phi_rns(t, phi + dphi, nu + dnu, sigma_n + dsigma, pnd, delay)
        ...
        k5 = h * phi_rns(t, phi + dphi, nu + dnu, sigma_n + dsigma, pnd, delay)
        ...
        k6 = h * phi_rns(t, phi + dphi, nu + dnu, sigma_n + dsigma, pnd, delay)
        l6 = h * th_rns(phi + dphi, nu + dnu)
        m6 = h * sigma_rns(phi + dphi, pnd)

        # Error estimation
        err = r1 * np.array([k1, l1, m1]) + r3 * np.array([k3, l3, m3]) + r4 * np.array([k4, l4, m4]) + r5 * np.array(
            [k5, l5, m5]) + r6 * np.array([k6, l6, m6])
...
if __name__ == "__main__":  # to allow import without running the simulation
    ...
    for i in range(N_taux + 1):
        ...
        for iter in range(0, pc.nitmax, 1):
            # --update phi, nu and h
            phi, nu, sigma_n, dphi, dnu, dsigma, h = rkf(t, phi, nu, sigma_n, f, h, pnd, pc, delay)
            ...
            Nu = np.append(Nu, [nu])
            Sigma_n = np.append(Sigma_n, [sigma_n - P(t, pd, pnd, delay)])
            F = np.append(F, [f_rns(phi, nu, pnd)])
            Tau = np.append(Tau, [f * (sigma_n - P(t, pd, pnd, delay))])
